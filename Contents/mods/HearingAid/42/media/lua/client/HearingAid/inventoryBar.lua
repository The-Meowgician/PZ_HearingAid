...
if HearingAidInventoryBar then
	if HearingAidInventoryBar.versionNumber >= versionNumber then
		return;
	end
end

HearingAidInventoryBar = {
	versionNumber = versionNumber,
	drawForItems = {},
};

HearingAidInventoryBar.registerItem = function(fullType, modData, label)
	HearingAidInventoryBar.drawForItems[fullType] = {
		modData = modData,
		label = label,
	};
end
...
HearingAidInventoryBar.doOverride = function()
	...
	ISInventoryPane.drawItemDetails = function(self, _item, _y, _xoff, _yoff, _red)
		if _item then
			local drawItem = HearingAidInventoryBar.drawForItems[_item:getFullType()];
			if drawItem then
...
HearingAidInventoryBar.init = function()
	HearingAidInventoryBar.doOverride();
end

Events.OnGameStart.Add(HearingAidInventoryBar.init);
